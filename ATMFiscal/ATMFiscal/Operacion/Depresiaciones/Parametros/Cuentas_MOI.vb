...
Public Class Cuentas_MOI
	...
	Private Sub CmdBuscarFact_Click(sender As Object, e As EventArgs) Handles CmdBuscarFact.Click
        ...
        Dim consulta As String = " SELECT Ctas_MOI.Id_Cta_MOI,   Ctas_MOI.Cta_Madre ,Convert(VARCHAR,Clave_Activos.Id_Clave,103) + '-' + Clave_Activos.Descripcion AS Tipo      FROM Ctas_MOI  INNER JOIN Clave_Activos ON Clave_Activos.Id_Clave =Ctas_MOI.Id_Clave INNER JOIN Empresa ON Empresa.Id_Empresa =Ctas_MOI.Id_Empresa WHERE Empresa.Id_Empresa  = " & Me.lstCliente.SelectItem & "  "
		...
		If ds.Tables(0).Rows.Count > 0 Then
			...
			For j As Integer = 0 To ds.Tables(0).Rows.Count - 1
				...
				End Try
				Me.TablaImportar.Item(ID.Index, j).Value = IIf(IsDBNull(ds.Tables(0).Rows(j)("Id_Cta_MOI")) = True, "", ds.Tables(0).Rows(j)("Id_Cta_MOI"))
	...
	Private Sub Edita_Registro(ByVal Id As Integer, ByVal Cta_Madre As String, ByVal Id_Clave As Integer)
		Dim sql As String = "UPDATE dbo.Ctas_MOI
                            SET  
	                        Cta_Madre = '" & Cta_Madre & "',Id_Clave = " & Id_Clave & "  where Id_Cta_MOI = " & Id & "  "
		If Eventos.Comando_sql(sql) > 0 Then
			Eventos.Insertar_usuariol("Ctas_MOIE", sql)
		End If
	End Sub
	Private Sub Inserta_Registro(ByVal Cta_Madre As String, ByVal Id_Clave As Integer, ByVal Id_Empresa As String)
		Dim sql As String = "INSERT INTO dbo.Ctas_MOI
	                        (
		...
		If Eventos.Comando_sql(sql) > 0 Then
			Eventos.Insertar_usuariol("Ctas_MOII", sql)
		End If
	End Sub
				...
				If Fila.Cells(CtaActivo.Index).Selected = True Then
					If RadMessageBox.Show("Realmente deseas eliminar los registros seleccionados?", Eventos.titulo_app, MessageBoxButtons.YesNo, RadMessageIcon.Question) = Windows.Forms.DialogResult.Yes Then
						If Me.TablaImportar.Item(ID.Index, Fila.Index).Value <> Nothing Then
							If Eventos.Comando_sql("Delete from dbo.Ctas_MOI where Id_Cta_MOI=" & Me.TablaImportar.Item(ID.Index, Fila.Index).Value) > 0 Then
								RadMessageBox.Show("Registro eliminado correctamente.", Eventos.titulo_app, MessageBoxButtons.OK, RadMessageIcon.Info)
								Eventos.Insertar_usuariol("Ctas_MOID", "Delete from dbo.Ctas_MOI where Id_Cta_MOI=" & Me.TablaImportar.Item(ID.Index, Fila.Index).Value & "")
							Else
	...
	Private Sub Cuentas_MOI_Load(sender As Object, e As EventArgs) Handles MyBase.Load
