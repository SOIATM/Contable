...
Public Class Cargos_Cero
    Dim Activo As Boolean
    Private Sub Diseño()
        Eventos.DiseñoTabla(Me.TablaImportar)
    End Sub
    Private Sub Cargos_Cero_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Activo = True
        Cargar()
        Diseño()
        Activo = False
    ...
    Private Sub CmdBuscarFact_Click(sender As Object, e As EventArgs) Handles CmdBuscarFact.Click
        ...
        If Me.LstEmpresa.SelectText <> "" Then
            ...
            Dim consulta As String = "SELECT Cargos_Cero.Id_Cargos_Cero,   Cargos_Cero.Alias "

            consulta &= " FROM     Cargos_Cero   where   Cargos_Cero.Id_Empresa = " & Me.LstEmpresa.SelectItem & " "
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                For j As Integer = 0 To ds.Tables(0).Rows.Count - 1
                    ...
                    Me.TablaImportar.Item(ID.Index, j).Value = IIf(IsDBNull(ds.Tables(0).Rows(j)("Id_Cargos_Cero")) = True, "", ds.Tables(0).Rows(j)("Id_Cargos_Cero"))
    ...
    Private Sub Edita_Registro(ByVal Id_Persona_Cliente As Integer, ByVal Ali As String, ByVal Cuenta As String)
        Dim sql As String = "UPDATE dbo.Cargos_Cero
                                SET  
    	                        Alias = '" & Ali & "',
    	                        Cuenta = '" & Cuenta & "'
    	                        where Id_Cargos_Cero = " & Id_Persona_Cliente & "  "
        If Eventos.Comando_sql(sql) > 0 Then
            Eventos.Insertar_usuariol("Cargos_CeroE", sql)
        End If
    End Sub
    Private Sub Inserta_Registro(ByVal Ali As String, ByVal Cuenta As String)
        Dim sql As String = "INSERT INTO dbo.Cargos_Cero
    	                        (
    	                        Cuenta,
    	                        Alias,
    	                        Id_Empresa 
                ...
                If Fila.Cells(Alia.Index).Selected = True Then
                    If RadMessageBox.Show("Realmente deseas eliminar los registros seleccionados?", Eventos.titulo_app, MessageBoxButtons.YesNo, RadMessageIcon.Question) = Windows.Forms.DialogResult.Yes Then
                        ...
                        If Me.TablaImportar.Item(ID.Index, Fila.Index).Value <> Nothing Then
                            If Eventos.Comando_sql("Delete from dbo.Cargos_Cero where Id_Cargos_Cero=" & Me.TablaImportar.Item(ID.Index, Fila.Index).Value) > 0 Then
